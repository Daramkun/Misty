...
namespace NVorbis
{
    abstract class VorbisMapping
    {
        internal static VorbisMapping Init(VorbisStreamDecoder vorbis, DataPacket packet)
        {
			...
			if ( mapping == null ) throw new ArgumentException ();
        ...
        class Mapping0 : VorbisMapping
        {
            ...
            protected override void Init(DataPacket packet)
            {
                ...
                // reserved bits
				if ( packet.ReadBits ( 2 ) != 0UL ) throw new ArgumentException ();
                ...
                if (submapCount > 1)
                {
                    for (int c = 0; c < ChannelSubmap.Length; c++)
                    {
						...
						if ( mux [ c ] >= submapCount ) throw new ArgumentException ();
                    }
                }
                ...
                for (int j = 0; j < submapCount; j++)
                {
					...
					if ( floorNum >= _vorbis.Floors.Length ) throw new ArgumentException ();
                    var residueNum = (int)packet.ReadBits(8);
					if ( residueNum >= _vorbis.Residues.Length ) throw new ArgumentException ();
