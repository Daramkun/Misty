...
using System.Linq;
using System.Text;
using Daramkun.Misty.IO.Compression.Algorithms;
using Daramkun.Misty.IO.Compression.Utilities;

namespace Daramkun.Misty.IO.Compression
{
	internal class ZlibBaseStream : Stream
	{
			...
			if ( streamMode == StreamMode.Writer )
			{
				...
				do
				{
					...
					if ( rc != ZlibConstants.Z_STREAM_END && rc != ZlibConstants.Z_OK )
					{
						...
						if ( _z.Message == null )
							throw new Exception ( String.Format ( "{0}: (rc = {1})", verb, rc ) );
						else
							throw new Exception ( verb + ": " + _z.Message );
					}
		...
		public override int Read ( byte [] buffer, int offset, int count )
		{
			if ( streamMode == StreamMode.Undefined )
			{
				if ( !this.stream.CanRead ) throw new Exception ( "The stream is not readable." );
			...
			do
			{
				...
				if ( rc != ZlibConstants.Z_OK && rc != ZlibConstants.Z_STREAM_END )
					throw new Exception ( String.Format ( "{0}flating:  rc={1}  msg={2}", ( _wantCompress ? "de" : "in" ), rc, _z.Message ) );
			...
			if ( _z.AvailableBytesOut > 0 )
			{
				...
				if ( nomoreinput )
				{
					if ( _wantCompress )
					{
						...
						if ( rc != ZlibConstants.Z_OK && rc != ZlibConstants.Z_STREAM_END )
							throw new Exception ( String.Format ( "Deflating:  rc={0}  msg={1}", rc, _z.Message ) );
					}
				}
			}
