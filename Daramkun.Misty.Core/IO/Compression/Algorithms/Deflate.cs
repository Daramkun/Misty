	...
	internal sealed class DeflateManager
	{
		...
		internal int Initialize ( ZlibCodec codec, CompressionLevel level, int windowBits, int memLevel, CompressionStrategy strategy )
		{
			...
			if ( windowBits < 9 || windowBits > 15 )
				throw new Exception ( "windowBits must be in the range 9..15." );

			if ( memLevel < 1 || memLevel > MEM_LEVEL_MAX )
				throw new Exception ( String.Format ( "memLevel must be in the range 1.. {0}", MEM_LEVEL_MAX ) );
		...
		internal int SetDictionary ( byte [] dictionary )
		{
			...
			if ( dictionary == null || status != INIT_STATE )
				throw new ArgumentException ( "Stream error." );
		...
		internal int Deflate ( FlushType flush )
		{
				...
				( status == FINISH_STATE && flush != FlushType.Finish ) )
			{
				...
				throw new Exception ( String.Format ( "Something is fishy. [{0}]", _codec.Message ) );
			}
			if ( _codec.AvailableBytesOut == 0 )
			{
				_codec.Message = _ErrorMessage [ ZlibConstants.Z_NEED_DICT - ( ZlibConstants.Z_BUF_ERROR ) ];
				throw new Exception ( "OutputBuffer is full (AvailableBytesOut == 0)" );
			}
			...
			{
				_codec.Message = _ErrorMessage [ ZlibConstants.Z_NEED_DICT - ( ZlibConstants.Z_BUF_ERROR ) ];
				throw new Exception ( "status == FINISH_STATE && _codec.AvailableBytesIn != 0" );
			}
